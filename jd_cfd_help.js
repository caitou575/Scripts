...
function submitCode(code, user) {
    return new Promise(async resolve => {
        $.get({url: `http://hz..top:99/share/submit/cfd?code=${code}&user=${user}`, timeout: 10000}, (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${JSON.stringify(err)}`)
...
function readShareCode() {
    return new Promise(async resolve => {
        $.get({url: `http://hz..top:99/share/get/cfd`, timeout: 60000,}, (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${JSON.stringify(err)}`)
                    console.log(`${$.name} API请求失败，请检查网路重试`)
