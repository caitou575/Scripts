/*
cron "0 9 * * *" jd_speed_sign_Part3.js, tag:京东极速版任务1
*/
...
!(async () => {
  ...
  let ckp3=Math.ceil(cookiesArr.length/10)*3;
  let ckp4=Math.ceil(cookiesArr.length/10)*4;
  
  if (ckp4>cookiesArr.length){
	  ckp4=cookiesArr.length;
  }
  if (ckp3>=ckp4){
	  console.log(`账号太少不需要第二个任务,跳出\n`);
	  return 
  }
  console.log(`本次执行第${ckp3+1}到${ckp4}个账号\n`);
  for (let i = ckp3 ; i < ckp4; i++) {
    if (cookiesArr[i]) {
      cookie = cookiesArr[i];
