...
!(async () => {
    ...
    let res = [];
    try{res = await getAuthorShareCode('https://raw.githubusercontt.com/l6/share_code/main/shop.json');}catch (e) {}
    if(!res){
        try{res = await getAuthorShareCode('https://giee.com/st7/shae-code/raw/master/shop.json');}catch (e) {}
        if(!res){res = [];}
    }
    if(res.length > 0){
