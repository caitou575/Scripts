/*
cron "0 14 * * *" jd_speed_sign_Part3.js, tag:京东极速版任务1
*/
 //详细说明参考 https://github.com/ccwav/QLScript2.
const $ = new Env('京东极速版任务9');
...
!(async () => {
  ...
  let ckp9=Math.ceil(cookiesArr.length/10)*9;
  
  if (ckp9>cookiesArr.length){
	  ckp9=cookiesArr.length;
  }
  if (ckp8>=ckp9){
	  console.log(`账号太少不需要第二个任务,跳出\n`);
	  return 
  }
  console.log(`本次执行第${ckp8+1}到${ckp9}个账号\n`);
  for (let i = ckp8 ; i < ckp9; i++) {
    if (cookiesArr[i]) {
      cookie = cookiesArr[i];
