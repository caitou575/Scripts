/*
cron "0 12 * * *" jd_speed_sign_Part3.js, tag:京东极速版任务1
*/
 //详细说明参考 https://github.com/ccwav/QLScript2.
const $ = new Env('京东极速版任务7');
...
!(async () => {
  ...
  let ckp6=Math.ceil(cookiesArr.length/10)*6;
  let ckp7=Math.ceil(cookiesArr.length/10)*7;
  
  if (ckp7>cookiesArr.length){
	  ckp7=cookiesArr.length;
  }
  if (ckp6>=ckp7){
	  console.log(`账号太少不需要第二个任务,跳出\n`);
	  return 
  }
  console.log(`本次执行第${ckp6+1}到${ckp7}个账号\n`);
  for (let i = ckp6 ; i < ckp7; i++) {
    if (cookiesArr[i]) {
      cookie = cookiesArr[i];
