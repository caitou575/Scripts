...
function readShareCode() {
  return new Promise(async resolve => {
    $.get({url: `https://raw.githubusercontent.com/caitou575/sharecodes/main/ddfactory.json`, 'timeout': 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
...
function shareCodesFormat() {
  return new Promise(async resolve => {
    ...
    $.newShareCodes = ["T0225KkcRkgYo1XUJhKnxf8PcQCjVWnYaS5kRrbA", "T0225KkcRBhN8lSFc0v9lvQLdwCjVWnYaS5kRrbA"];
    ...
    if (readShareCodeRes) {
      ...
      rdmsharecodes = await getRandomArrayElements(readShareCodeRes.data,Math.ceil(cookiesArr.length/5) + 2 );//随机抽n个取助力码
