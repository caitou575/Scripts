...
function readShareCode() {
  return new Promise(async resolve => {
    $.get({url: `https://raw.githubusercontent.com/caitou575/sharecodes/main/jd_pet.json`, 'timeout': 10000}, (err, resp, data) => {
      try {
        if (err) {
...
function shareCodesFormat() {
  return new Promise(async resolve => {
    ...
    newShareCodes = ["MTAxODc2NTEzNDAwMDAwMDAyMDQzMjkyMw==","MTE1NDUwMTI0MDAwMDAwMDQ0MTUxMDMz","MTE1NDUwMTI0MDAwMDAwMDQ0NTg5NDI1","MTEzMzI0OTE0NTAwMDAwMDA0NTA1MjI4Nw=="];
    ...
    if (readShareCodeRes) {
      ...
      rdmsharecodes = await getRandomArrayElements(readShareCodeRes.data,Math.ceil(cookiesArr.length / 1) + 4);//随机抽n个取助力码
