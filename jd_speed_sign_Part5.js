/*
cron "0 10 * * *" jd_speed_sign_Part3.js, tag:京东极速版任务1
*/
...
!(async () => {
  ...
  let ckp4=Math.ceil(cookiesArr.length/10)*4;
  let ckp5=Math.ceil(cookiesArr.length/10)*5;
  
  if (ckp5>cookiesArr.length){
	  ckp5=cookiesArr.length;
  }
  if (ckp4>=ckp5){
	  console.log(`账号太少不需要第二个任务,跳出\n`);
	  return 
  }
  console.log(`本次执行第${ckp4+1}到${ckp5}个账号\n`);
  for (let i = ckp4 ; i < ckp5; i++) {
    if (cookiesArr[i]) {
      cookie = cookiesArr[i];
