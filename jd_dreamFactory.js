...
async function joinLeaderTuan() {
  let res = await updateTuanIdsCDN('https://raw.githubusercontent.com/1Aaron-lv/updateTeam/master/shareCodes/jd_updateFactoryTuanId.json')
  if (!res) {
    $.http.get({url: 'https://purge.jsdelivr.net/gh/1Aaron-lv/updateTeam@master/shareCodes/jd_updateFactoryTuanId.json'}).then((resp) => {}).catch((e) => $.log('刷新CDN异常', e));
    await $.wait(1000)
    res = await updateTuanIdsCDN('https://cdn.jsdelivr.net/gh/1Aaron-lv/updateTeam@master/shareCodes/jd_updateFactoryTuanId.json');
  }
...
function readShareCode() {
  console.log(`开始`)
  return new Promise(async resolve => {
    $.get({url: `https://raw.githubusercontent.com/caitou575/sharecodes/main/jd_dreamfactory.json`, 'timeout': 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
...
function shareCodesFormat() {
  return new Promise(async resolve => {
    ...
    $.newShareCodes = ["03ICP_y5hLezmikoeuj0yA==","0O052E-tXswa9ZoGVBL21A==","F0z7jG6kPobqxpJhBAx43A==","HMzIgGHgTYb_Dy4nActNxA=="];
    ...
    if (readShareCodeRes) {
      readShareCodeRes.data = readShareCodeRes.data.filter(function(n){return n});//去除空值
      rdmsharecodes = await getRandomArrayElements(readShareCodeRes.data,Math.ceil(cookiesArr.length * 3 / 20) + 4);//随机抽n个取助力码
      $.newShareCodes = [...new Set([...$.newShareCodes, ...(rdmsharecodes || [])])];//添加固定助力码后去重生成最终助力池
    }
    console.log(`本次将要助力的好友${JSON.stringify($.newShareCodes)}`)
    resolve();
  })
}
//随机抽取助力码
