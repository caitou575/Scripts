/*
cron "0 13 * * *" jd_speed_sign_Part3.js, tag:京东极速版任务1
*/
 //详细说明参考 https://github.com/ccwav/QLScript2.
const $ = new Env('京东极速版任务8');
...
!(async () => {
  ...
  let ckp7=Math.ceil(cookiesArr.length/10)*7;
  let ckp8=Math.ceil(cookiesArr.length/10)*8;
  
  if (ckp8>cookiesArr.length){
	  ckp8=cookiesArr.length;
  }
  if (ckp7>=ckp8){
	  console.log(`账号太少不需要第二个任务,跳出\n`);
	  return 
  }
  console.log(`本次执行第${ckp7+1}到${ckp8}个账号\n`);
  for (let i = ckp7 ; i < ckp8; i++) {
    if (cookiesArr[i]) {
      cookie = cookiesArr[i];
