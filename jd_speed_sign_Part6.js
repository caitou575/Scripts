/*
cron "0 11 * * *" jd_speed_sign_Part3.js, tag:京东极速版任务1
*/
 //详细说明参考 https://github.com/ccwav/QLScript2.
const $ = new Env('京东极速版任务6');
...
!(async () => {
  ...
  let ckp5=Math.ceil(cookiesArr.length/10)*5;
  let ckp6=Math.ceil(cookiesArr.length/10)*6;
  
  if (ckp6>cookiesArr.length){
	  ckp6=cookiesArr.length;
  }
  if (ckp5>=ckp6){
	  console.log(`账号太少不需要第二个任务,跳出\n`);
	  return 
  }
  console.log(`本次执行第${ckp5+1}到${ckp6}个账号\n`);
  for (let i = ckp5 ; i < ckp6; i++) {
    if (cookiesArr[i]) {
      cookie = cookiesArr[i];
